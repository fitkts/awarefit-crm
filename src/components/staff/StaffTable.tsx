...
const StaffTable: React.FC<StaffTableProps> = ({
  ...
  // 일괄 작업 옵션
  const bulkActions: { type: StaffBulkAction; label: string; icon: string; requiresConfirmation: boolean; confirmMessage: string }[] = [
    {
      type: 'activate',
      label: '활성화',
      icon: 'UserCheck',
  ...
  const renderCell = (staffMember: Staff, column: StaffTableColumn) => {
    switch (column.key) {
      ...
      case 'position':
        return (
          <div className="flex items-center">
            ...
            <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${staffMember.position === '매니저'
      ...
      case 'is_active':
        return (
          <span
            className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${staffMember.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
              }`}
          >
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            <button
              onClick={() => onBulkAction('activate', staff.map(s => s.id))}
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
            ...
            <tr>
              ...
              {columns
                ...
                .map((column) => (
                  <th
                    key={column.key}
                    className={`px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider ${column.sticky ? 'sticky bg-gray-50' : ''
                      }`}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? (
              <tr>
                <td colSpan={columns.filter(c => c.visible).length + 1} className="px-4 py-8 text-center">
              ...
              staff.map((staffMember) => (
                <tr
                  key={staffMember.id}
                  className={`hover:bg-gray-50 ${selectedStaff.includes(staffMember.id) ? 'bg-blue-50' : ''
                    }`}
                >
                  <td className="w-8 px-4 py-3">
                    <button onClick={() => handleSelectStaff(staffMember.id)} className="flex items-center">
