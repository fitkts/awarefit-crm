            ...
            <div className="space-y-6">
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700">결제 유형</label>
                    <p className="mt-1 text-sm text-gray-900">{getPaymentTypeText(payment.payment_type)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-lg font-bold text-gray-900">{formatCurrency(payment.amount)}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">결제 방식</label>
                    <p className="mt-1 text-sm text-gray-900">{payment.payment_method}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{payment.member_name}</p>
                  </div>
                  <div>
                  ...
                  {payment.member_phone && (
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-900">{payment.member_phone}</p>
                    </div>
                  )}
                  <div>
                    <label className="block text-sm font-medium text-gray-700">담당 직원</label>
                    <p className="mt-1 text-sm text-gray-900">{payment.staff_name}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                {payment.membership_type_name && (
                  <div className="mb-3">
                    ...
                    <p className="mt-1 text-sm text-gray-900">{payment.membership_type_name}</p>
                  </div>
                )}
                {payment.pt_package_name && (
                  <div className="mb-3">
                    <label className="block text-sm font-medium text-gray-700">PT 패키지</label>
                    <p className="mt-1 text-sm text-gray-900">{payment.pt_package_name}</p>
                  </div>
                )}
                {payment.expiry_date && (
                  <div className="mb-3">
                    <label className="block text-sm font-medium text-gray-700">만료일</label>
                    <p className="mt-1 text-sm text-gray-900">{formatDate(payment.expiry_date)}</p>
                  </div>
                )}
                {payment.notes && (
                  <div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {payment.refunds.map((refund) => (
                      <div key={refund.id} className="bg-yellow-50 p-3 rounded-lg">
                        <div className="flex justify-between items-start">
                          ...
                          <span className={`inline-flex items-center px-2 py-1 text-xs font-medium rounded-full ${refund.status === 'processed' ? 'bg-green-100 text-green-800' :
                    ...
                    {payment.history.map((history) => (
                      <div key={history.id} className="bg-gray-50 p-3 rounded-lg">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="text-sm font-medium text-gray-900">
                              {history.action === 'created' ? '결제 생성' :
                                history.action === 'updated' ? '결제 수정' :
                                  history.action === 'refund_requested' ? '환불 신청' :
                                    history.action === 'refunded' ? '환불 완료' :
                                      history.action === 'cancelled' ? '결제 취소' : history.action}
                            </p>
                            {history.notes && (
