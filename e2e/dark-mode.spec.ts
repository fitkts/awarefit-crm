...
test.describe('다크모드 기능', () => {
  ...
  test('대시보드에서 다크모드 토글이 올바르게 작동해야 함', async ({ page }) => {
    await page.goto('http://localhost:3000');
    
    // 대시보드가 기본 페이지이므로 바로 확인
    const sidebar = page.locator('nav').first();
    await sidebar.hover();
    const themeToggle = page.locator('button[aria-label*="다크모드"], button[title*="다크모드"]').first();
    await expect(themeToggle).toBeVisible();
  ...
  test('회원/결제/직원/대시보드에서 다크모드 스타일이 유지되어야 함', async ({ page }) => {
    await page.goto('http://localhost:3000');
    ...
    await themeToggle.click();
    await expect(page.locator('html')).toHaveClass(/dark/);
    // 각 주요 페이지로 이동하며 다크모드 유지 확인
    for (const label of ['회원 관리', '결제 관리', '직원 관리', '대시보드']) {
